...
public class MenuHandler {
    ...
    private final IBookingDao bookdao=new IBookingDaoImpl();

    public MenuHandler(UserFactory userFactory) {
    ...
    private void listAllBookingsFromDB() throws Exception {

        List<Booking> bookings = bookdao.findAll(); // fetch from DB
    ...
    private void getBookingByIdFromDB() throws Exception {

        System.out.print("Enter Booking ID: "); String id = sc.nextLine().trim();
        Booking b = bookdao.findById(id); // DB call
    ...
    private void getTaxiByIdFromDB(ITaxiDaoProxy proxy) throws Exception {
        ...
        Taxi t = proxy.findById(id); // DB call via proxy
        if (t != null) System.out.println("Taxi details: " + t);
        else System.out.println("Taxi not found.");
    }

    private void viewRiderBookingsFromDB(Rider rider) throws Exception {
        List<Booking> bookings = bookdao.findAll().stream()
